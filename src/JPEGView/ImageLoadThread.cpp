...
/*
* Verify if given index is valid against invalid ones marked in zipIgnoreIndices,
* and change it to the next valid index if needed.
*/
bool CImageLoadThread::ValidateZipFrameIndex(int nInitialIndex, int& nFrameIndex)
{
	if (nFrameIndex >= m_nZipCount)
		nFrameIndex = 0; //rollover
	while (zipIgnoreIndices.count(nFrameIndex) > 0)
	{
		++nFrameIndex;
		if (nFrameIndex >= m_nZipCount)
			nFrameIndex = 0;
		if (nFrameIndex == nInitialIndex)
			return false;
	}
	return true;
}
...
void CImageLoadThread::ProcessReadZipRequest(CRequest* request) {
	...
	if (sFileName == m_sLastZipFileName) {
		if ((m_nZipCount - zipIgnoreIndices.size()) <= 0)
		...
		if (nFrameIndex < 0)
			nFrameIndex = 0;
		if (!ValidateZipFrameIndex(nFrameIndex, nFrameIndex))
			return;
	...
	int nInitialFrameIndex = nFrameIndex;
	char* pBuffer = NULL;
	do {
		try {
		...
		if (!bSuccess)
		{
			zipIgnoreIndices[nFrameIndex] = true;
			//try next file in archive
			++nFrameIndex;
			if (!ValidateZipFrameIndex(nInitialFrameIndex, nFrameIndex))
			{
				break; //abort
			}
